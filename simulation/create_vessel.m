function opts = create_vessel(opts)
  ...
  for i=1:opts.init_params(1)
    ...
    new_vessel = get_vessel(bounding_box, vessel_props, opts.init_params(2));
  ...
  figure;
  [p,t]=distmesh_poly(vessels.',min(opts.image_size)/10,bounding_box([1 3; 2 4]));
...
function vessel = get_vessel(bounding_box, props, nbranching)

  shifted = bounding_box([2 4]) + bounding_box([1 3]);
  %len = sqrt(2*sum(shifted.^2));
  len = sqrt(0.25*sum(shifted.^2));
  angle = rand(1)*2*pi;
  ...
  if (nbranching == 0)
    ...
    for i=1:nbranching
      ...
      [new_middle, new_vessel] = get_bounding_box(lens(1), angles(1), curr_width(1), center);
      middle = [middle NaN(2,1) new_middle];
      [x,y] = polybool('|', vessel(1,:), vessel(2,:), new_vessel(1,:), new_vessel(2,:));
      vessel = [x;y];

      [new_middle, new_vessel] = get_bounding_box(lens(2), angles(2), curr_width(2), center);
      middle = [middle NaN(2,1) new_middle];
      [x,y] = polybool('|', vessel(1,:), vessel(2,:), new_vessel(1,:), new_vessel(2,:));
      vessel = [x;y];
