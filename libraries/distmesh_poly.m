function [p,t]=distmesh_poly(poly,h0,bbox)
...
p=p(fd(p,poly)<geps,:);                 % Keep only d<0 points
...
p=setdiff(p,poly,'rows');
pfix=unique(poly,'rows');
pfix = pfix(~any(isnan(pfix),2), :);
nfix=size(pfix,1);
...
while 1
  ...
  if max(sqrt(sum((p-pold).^2,2))/h0)>ttol           % Any large movement?
    ...
    t=t(fd(pmid,poly)<-geps,:);         % Keep interior triangles
  ...
  L0=Fscale*sqrt(sum(L.^2)/numel(L));     % L0 = Desired lengths
  ...
  damping = exp(-(count^2)/(2*timing^2));
  p=p+(deltat*damping)*Ftot;                                   % Update node positions
  ...
  d=fd(p,poly); ix=d>0;                 % Find points outside (d>0)
  dgradx=(fd([p(ix,1)+deps,p(ix,2)],poly)-d(ix))/deps; % Numerical
  dgrady=(fd([p(ix,1),p(ix,2)+deps],poly)-d(ix))/deps; %    gradient
  dgrad2=dgradx.^2+dgrady.^2;
  proj=[d(ix).*dgradx./dgrad2,d(ix).*dgrady./dgrad2];    % Project
  p(ix,:)=p(ix,:)-proj;    % Project

  % 8. Termination criterion: All interior nodes move less than dptol (scaled)
  %if max(sqrt(sum(deltat*Ftot(d<-geps,:).^2,2))/h0)<dptol, break; end
  if nfix==N || (all(abs(d) < geps) && all(sqrt(sum(deltat*damping*Ftot.^2,2))/h0 < dptol)) ...
             || (any(d<-geps) && max(sqrt(sum(deltat*damping*Ftot(d<-geps,:).^2,2))/h0)<dptol)
    break;
  end
end
