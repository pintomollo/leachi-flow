...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  double steepness = 1, norm, dist;
  bool free_memory = false;

  // Check for proper number of input and output arguments
  if (nrhs < 2) {
    mexErrMsgIdAndTxt("MATLAB:draw_bumps:invalidInputs",
        "Not the proper number of input arguments (2 or 3 are expected) !");
  } else if (nrhs == 3) {
    steepness = mxGetScalar(prhs[2]);
  }
  steepness = -fabs(steepness);
  norm = 1 / fast_exp(steepness);

  // Accept only 2D data
  ndim = mxGetNumberOfElements(prhs[0]);
  if (ndim != 2) {
    mexErrMsgIdAndTxt("MATLAB:draw_bumps:invalidDimensions",
  ...
  if (n > 0) {
    ...
    if (nprops < 1) {
      mexErrMsgIdAndTxt("MATLAB:draw_gaussians:invalidGaussians",
          "Not enough parameters of the Bump spots are provided !");
    } else if (nprops == 1) {
    ...
    for (i=1; i <= w; i++) {
      for (j=1; j <= h; j++) {
        for (c=0; c < n; c++) {
          dist = sigma[c] * (__SQR__((double)i - x[c]) + __SQR__((double)j - y[c]));
          img[j-1] += (dist < 1 ? ampl[c] * norm * fast_exp(steepness / (1 - dist)) : 0);
        }
      }
