...
% 13.05.2014
  ...
  switch type
    ...
    case 'botrylloides_leachi'
      ...
      vessel = gmm;
      vessel.mu = [14.54; 20.63];
      vessel.sigma = cat(3, 7.38, 1.87);
      vessel.proportions = [0.69 0.31];

      % The radius / intensity of cells
      cells = gmm;
      cells.mu = [2.22 0.28; 1.78 0.11];
      cells.sigma = cat(3, [0.133 -0.018; -0.018 0.003], [0.084 0.002; 0.002 0.002]);
      cells.proportions = [0.12 0.88];

      % The radius / intensity of ampullae
      ampullae = gmm;
      %ampullae.mu = [18.96 0.36];
      ampullae.mu = [2*18.96 2.5*0.36];
      ampullae.sigma = [4.12, 0.015];
      ampullae.proportions = 1;

      % The radius / intensity of immobile cells
      spots = gmm;
      spots.mu = [2 0.26];
      spots.sigma = [0.16, 0.028];
      spots.proportions = 1;
