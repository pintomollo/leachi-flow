...
parameters.ImageFolder                      = '/Users/simonblanchoud/Documents/Movies/Sectioning/TrialColony/Section008'; % absolute or relative path where the images are stored inside the computer. E.g.: 'INPUTIMAGES\' or 'C:\AbsolutePath\INPUTIMAGES\'.
parameters.ImageBaseName                    = 'Section008_'; % name of the images to be stitched, without the final cardinal number. The last character must be the underscore. E.g.: 'ImageMesenchymal_' for the image: "ImageMesenchymal_002.tif". The images must be saved (or copied) in the following format: "Name_#...##.format".
...
parameters.flag_Color                       = 0; % (by default: 1). 0 to obtain the final mosaic in grey levels (also if the original images are RGB). 1 to obtain the final mosaic in RGB (only if the oiginal images are RGB). 
parameters.flag_SeekBestImages              = 1; % (by default: 1). The input frame are pre-selected to optimize the registration task. If this parameter is set to 1, not all the images pointed out are stitched in the final mosaic.
