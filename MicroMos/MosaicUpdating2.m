function [Mosaic, MosaicOrigin, MaskOverlap, Corner_Position] = MosaicUpdating2(Mosaic, unregistered, GLOBAL, flag_Blending, MosaicOrigin, MaskOverlap, InterpolationMode, RegistrationMode, index, Corner_Position)
...
%YmaxI = ceil(Ymax);
XminI = 0;
XmaxI = columnsMosaic-1;
YminI = 0;
YmaxI = rowsMosaic-1;

unregisteredWarped = myimtransform(unregistered, modello, GLOBAL, [XminI XmaxI]-MosaicOrigin(1), [YminI YmaxI]-MosaicOrigin(2));
...
%MosaicOrigin = floor([min([MosaicOrigin(1),ULC(1),DLC(1)]), min([MosaicOrigin(2),ULC(2),URC(2)])]);
MosaicOrigin = MosaicOrigin + ULC(1:2).';
