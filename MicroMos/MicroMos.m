...
% Simon Blanchoud, Wilson lab, University of Otago, 2015
  ...
  countMos = 1;
  while start_index < length(parameters.ImageIndexs)
    ...
    referenceFrame  = uint8(norm_factor*double(referenceFrame));
      ...
      while index < stop_index
          ...
          while flag_Problem == 1
              ...
              if parameters.flag_PhaseCorrelationOnly == 1
                  ...
                  if (parameters.flag_Color==0)
                      [PointsBase, PointsTracked] = PointsDetectionTracking(base, unregistered, parameters.numberCorners, parameters.flag_Harris, parameters.ShiftEstimationMode, parameters.flag_PCglobalORlocal, parameters.PCscaleFactor);
                  else
                      ba = rgb2gray(uint8(base));
                      un = rgb2gray(uint8(unregistered));
                      ba = fPixelAccuracy(ba);
                      un = fPixelAccuracy(un);
                      [PointsBase, PointsTracked] = PointsDetectionTracking(ba, un, parameters.numberCorners, parameters.flag_Harris, parameters.ShiftEstimationMode, parameters.flag_PCglobalORlocal, parameters.PCscaleFactor);
                      clear ba un
                  end
    ...
    for i=1:NumberOfregisteredImages
        ...
        if parameters.flag_FrameToMosaic == 1 && i>1
            ...
            [PointsBase, PointsTracked] = PointsDetectionTracking(double(regionOverlapped), double(newunregistered), parameters.numberCorners, parameters.flag_Harris, 0, parameters.flag_PCglobalORlocal, parameters.PCscaleFactor);
