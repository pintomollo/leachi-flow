function [H, inliers] = RansacTranslative(x1, x2, t)
...
s = 1;  % Minimum No of points needed to fit a translative.
...
if npts < 1
    error('Must have at least 1 points to fit translative');
end
...
% be inliers.
H = TraslativeModel2D(x1(:,inliers), x2(:,inliers));
...
function Ha = TraslativeModel2D(varargin)
    
[x1, x2] = checkargs(varargin(:));

flag_normalization = 0;

if flag_normalization == 1 && size(x1,2) ~= 1
